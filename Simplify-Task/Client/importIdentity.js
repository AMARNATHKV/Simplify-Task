...
async function importIdentity(userType, userName) {
    try {
        ...
        const credPath = path.join(__dirname, `../Network/organizations/peerOrganizations/org1.task.com/users/${userType}@org1.task.com/msp`);
...
async function main() {
    ...
    await importIdentity("Admin", "org1admin");
    await importIdentity("Auditor", "org1auditor");
    await importIdentity("User", "org1user");
